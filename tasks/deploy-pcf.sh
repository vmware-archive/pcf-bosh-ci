...
bosh -n deploy cf-deployment/cf-deployment.yml \
  ...
  --var notifications_ui_release_version="$(cat "file://$PWD/notifications-ui-release/version")" \
  --var cf_autoscaling_release_path="file://$PWD/cf-autoscaling-release/cf-autoscaling*.tgz" \
  --var cf_autoscaling_release_version="$(cat "file://$PWD/cf-autoscaling-release/version")" \
  --var console_release_path="file://$PWD/console-release/*.tgz" \
  --var console_release_version="$(cat "file://$PWD/console-release/version")" \
  --var cf_release_path="file://$PWD/cf-release/*.tgz" \
  --var postgres_release_path="file://$PWD/postgres-release/*.tgz" \
  --var cf_release_version="$(cat "file://$PWD/cf-release/version")" \
  --var postgres_release_version="$(cat "file://$PWD/postgres-release/version")" \
